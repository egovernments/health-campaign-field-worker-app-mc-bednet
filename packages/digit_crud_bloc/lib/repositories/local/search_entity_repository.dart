...
class SearchEntityRepository extends LocalRepository {
  ...
  Future<(Map<String, List<EntityModel>>, int)> _executeMultiTableSearch({
    ...
    if (filterLogic == MultiTableFilterLogic.or &&
      ...
      crossTableConstraints = [crossTableConstraints.reduce((a, b) => a | b)];
    }
    debugPrint('[SearchPerf] buildConstraints=${stepSw.elapsedMilliseconds}ms '
    ...
    debugPrint('[SearchPerf] primaryQuery=${stepSw.elapsedMilliseconds}ms '
    ...
    debugPrint('[SearchPerf] hydratePrimary=${stepSw.elapsedMilliseconds}ms');
    ...
    debugPrint('[SearchPerf] expandRelated=${stepSw.elapsedMilliseconds}ms '
    ...
    debugPrint('[SearchPerf] convertEntities=${stepSw.elapsedMilliseconds}ms');

    debugPrint('[SearchPerf] TOTAL=${overallSw.elapsedMilliseconds}ms '
  ...
  Future<List<Map<String, dynamic>>> _traverseRelationshipPath({
    ...
    for (var i = 0; i < path.length; i++) {
      ...
      final List<String>? hopProjection =
          isLastStep ? null : [path[i + 1].localKey];
