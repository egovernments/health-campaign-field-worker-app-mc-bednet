...
class MultiTableFilterResolver {
  ...
  Future<MultiTableFilterResult> resolveFilters({
    ...
    final relatedTablesWithFilters =
        filtersByTable.keys.where((table) => table != primaryTable).toList();
  ...
  Future<
      ({
        List<SearchFilter> primaryTableFilters,
  ...
  Future<Set<dynamic>> _traversePathToPrimaryKeys({
    ...
    for (var i = 0; i < path.length; i++) {
      ...
      if (joinValues.isEmpty) {
        _log(
            'No join values found at step ${i + 1} (${rel.from} -> ${rel.to})');
        return {};
      }
      ...
      final nextStepProjection =
          isLastStep ? primaryKeyField : path[i + 1].localKey;
